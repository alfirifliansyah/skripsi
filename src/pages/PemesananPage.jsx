  ...
  return new Promise((resolve, reject) => {
    ...
    if (!clientKey || !snapUrl) {
      reject(new Error(
        ...
        "Tambahkan VITE_MIDTRANS_CLIENT_KEY dan VITE_MIDTRANS_SNAP_URL ke file .env"
      ));
      return;
    }
    ...
    if (existing) {
      const poll = setInterval(() => {
        if (window.snap) { clearInterval(poll); resolve(window.snap); }
      }, 100);
      return;
    }
    ...
    script.onerror = () => reject(new Error("Gagal memuat Midtrans Snap.js. Periksa koneksi internet Anda."));
...
export default function PemesananPage({
  ...
  const handleManualRefresh = async () => {
    ...
    try {
      await onRefresh();
      setToast({ type: "success", msg: "Daftar pesanan diperbarui" });
    } catch {
      setToast({ type: "error", msg: "Gagal memperbarui daftar" });
    } finally {
      setRefreshing(false);
    }
  };
  ...
  const handlePay = async (ord) => {
    ...
    try {
      ...
      snap.pay(snap_token, {
        ...
        onPending(result) {
          setToast({ type: "success", msg: "Menunggu konfirmasi pembayaran…" });
          if (onRefresh) onRefresh();
          console.log("Midtrans pending:", result);
        },
        onError(result) {
          setToast({ type: "error", msg: "Pembayaran gagal, coba lagi." });
          ...
          >
            ...
            >
              <div style={{ height:140, background:svc?.imgBg || "linear-gradient(135deg,#1B4FD8,#23d5ab)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:56, position:"relative" }}>
                ...
                <button
                  onClick={() => setActiveOrder(null)}
                  style={{ position:"absolute", top:14, left:14, background:"rgba(255,255,255,.2)", border:"none", color:WHITE, width:32, height:32, borderRadius:"50%", cursor:"pointer", fontSize:16, display:"flex", alignItems:"center", justifyContent:"center" }}
                >←</button>
                <span style={{ position:"absolute", top:14, right:14, background:st.bg, color:st.color, fontSize:11, fontWeight:700, padding:".3rem .8rem", borderRadius:100, border:`1px solid ${st.color}33` }}>
                  {st.icon} {st.label}
                </span>
              </div>
              ...
              <div className="modal-card" style={{ padding:"1.75rem" }}>
                <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:"1.25rem", flexWrap:"wrap", gap:8 }}>
                  ...
                  <span style={{ fontSize:12, color:MUTED, fontFamily:"monospace", background:BG, padding:".3rem .7rem", borderRadius:8, border:"1px solid #E5EAF5" }}>
                    {ord.kode || ord.id}
                  </span>
                </div>
                ...
                <div className="btn-group" style={{ marginTop:"1.5rem", display:"flex", gap:".75rem", flexWrap:"wrap" }}>
                  ...
                  {needsPayment(ord) && (
                    ...
                    >
                      {fetchingToken
                        ? <><span style={{ animation:"spin 1s linear infinite", display:"inline-block" }}>⏳</span> Menyiapkan…</>
                        : "💳 Bayar Sekarang"
                      }
                    </button>
                  )}
                  ...
                  {svc && (
                    <button
                      onClick={() => { setActiveOrder(null); onGoBooking(svc); }}
                      style={{ flex:1, minWidth:120, padding:".75rem", background:BLUE, color:WHITE, border:"none", borderRadius:10, fontWeight:700, fontSize:14, cursor:"pointer", fontFamily:"inherit" }}
                    >
                      Pesan Lagi
                    </button>
                  )}
                  ...
                  <button
                    onClick={() => setActiveOrder(null)}
                    style={{ padding:".75rem 1.25rem", background:WHITE, color:DARK, border:"1.5px solid #D1D9EF", borderRadius:10, fontWeight:500, fontSize:14, cursor:"pointer", fontFamily:"inherit" }}
                  >
                    Tutup
                  </button>
                </div>
      ...
      <div className="section-pad" style={{ maxWidth:1760, margin:"0 auto", padding:"7rem 2rem 5rem" }}>
        <Anim>
          ...
          <p style={{ fontSize:15, color:MUTED, marginBottom:"2rem" }}>
            Pantau status dan riwayat semua pesanan jasa Anda di sini. Status diperbarui otomatis setiap 12 detik.
          </p>
        </Anim>
        ...
        <Anim delay={0.06}>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:"1rem", marginBottom:"2rem" }}>
            ...
            <button
              onClick={onGoJasa}
              style={{ background:BLUE, color:WHITE, border:"none", padding:".6rem 1.4rem", borderRadius:8, cursor:"pointer", fontWeight:700, fontSize:13, fontFamily:"inherit", display:"flex", alignItems:"center", gap:6 }}
            >
              + Pesan Jasa Baru
            </button>
          </div>
        ...
        {ordersLoading && orders.length === 0 ? (
          ...
          <Anim>
            <div className="card-pad" style={{ textAlign:"center", padding:"5rem", color:MUTED, background:WHITE, borderRadius:20, border:"1.5px solid #E5EAF5" }}>
              ...
              <p style={{ fontSize:14, marginBottom:"2rem" }}>
                {orders.length === 0
                  ? "Mulai pesan layanan multimedia profesional dari IMA Creative Production."
                ...
                <Anim key={ord.id_pemesanan || ord.id || i} delay={i * 0.04}>
                  ...
                  >
                    ...
                    <div style={{ textAlign:"right", flexShrink:0 }}>
                      ...
                      <span style={{ background:st.bg, color:st.color, fontSize:11, fontWeight:700, padding:".3rem .8rem", borderRadius:100, border:`1px solid ${st.color}33`, whiteSpace:"nowrap" }}>
                        {st.icon} {st.label}
                      </span>
                    </div>
