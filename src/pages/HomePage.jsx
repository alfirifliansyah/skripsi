...
export default function HomePage({
  ...
  // Helper untuk ambil nilai pengaturan dengan fallback
  ...
  // Pakai jasa dari API; fallback ke statis kalau belum ada
  ...
  return (
    <div style={{ fontFamily:"'DM Sans','Segoe UI',sans-serif", background:BG, color:DARK, overflowX:"hidden" }}>
      ...
      <section style={{ minHeight:"100vh", display:"flex", alignItems:"center", paddingTop:68, position:"relative", overflow:"hidden" }}>
        <div style={{ position:"absolute", top:"10%", right:"2%", width:440, height:440, borderRadius:"50%", background:BLUE_L, zIndex:0 }}/>
        <div style={{ position:"absolute", bottom:"5%", left:"-4%", width:220, height:220, borderRadius:"50%", background:YELLOW_L, zIndex:0 }}/>
        <div style={{ position:"absolute", top:0, left:0, right:0, height:4, background:`linear-gradient(90deg,${BLUE},${YELLOW})` }}/>
        <div style={{ maxWidth:1920, margin:"0 auto", padding:"5rem 3rem", display:"grid", gridTemplateColumns:"1fr 1fr", gap:"4rem", alignItems:"center", width:"100%", position:"relative", zIndex:1 }}>
          <Anim>
            <Label>{t("hero_label", "PRODUCTION HOUSE TERPERCAYA")}</Label>
            <h1 style={{ fontSize:"clamp(2.4rem,4.5vw,3.5rem)", fontWeight:700, lineHeight:1.15, marginBottom:"1.4rem", color:DARK, letterSpacing:"-.02em" }}>
              {t("hero_title", "Solusi Kreatif & Teknologi untuk Bisnis Anda")}
            </h1>
            <p style={{ fontSize:16, color:MUTED, lineHeight:1.85, marginBottom:"2.5rem", maxWidth:460 }}>
          ...
          <Anim delay={0.15}>
            <div style={{ background:WHITE, borderRadius:20, padding:"2rem", boxShadow:"0 8px 40px rgba(27,79,216,.1)", border:"1px solid #E5EAF5" }}>
      ...
      {/* ── STATS BAR ── */}
      <section style={{ background:BLUE, padding:"2.5rem 3rem" }}>
        <div style={{ maxWidth:1920, margin:"0 auto", display:"grid", gridTemplateColumns:"repeat(4,1fr)", textAlign:"center" }}>
          {[
      ...
      {/* ── ABOUT ── */}
      <section style={{ padding:"7rem 3rem", background:WHITE }}>
        <div style={{ maxWidth:1920, margin:"0 auto", display:"grid", gridTemplateColumns:"1fr 1fr", gap:"5rem", alignItems:"center" }}>
          <Anim>
            <div style={{ position:"relative" }}>
              <div style={{ background:BLUE_L, borderRadius:24, padding:"3rem", display:"flex", alignItems:"center", justifyContent:"center", aspectRatio:"1", maxWidth:340, margin:"0 auto" }}>
                <div style={{ width:150, height:150, background:BLUE, borderRadius:"50%", display:"flex", alignItems:"center", justifyContent:"center", fontSize:60 }}>🎬</div>
              </div>
          ...
          <Anim delay={0.15}>
            ...
            <h2 style={{ fontSize:"clamp(1.8rem,3vw,2.5rem)", fontWeight:700, lineHeight:1.25, marginBottom:"1.25rem", color:DARK }}>{t("about_title", "Inovasi & Kreativitas Tanpa Batas")}</h2>
      ...
      {/* ── DIRECTOR MESSAGE ── */}
      <section style={{ padding:"6rem 3rem", background:BG, borderTop:"1px solid #E5EAF5", borderBottom:"1px solid #E5EAF5" }}>
        <Anim>
          <div style={{ maxWidth:720, margin:"0 auto", textAlign:"center" }}>
            ...
            <blockquote style={{ fontFamily:"'DM Serif Display',serif", fontStyle:"italic", fontSize:"clamp(1.2rem,2.5vw,1.7rem)", lineHeight:1.65, color:DARK, margin:"0 0 2rem" }}>
        ...
        <div style={{ maxWidth:1920, margin:"0 auto" }}>
          <Anim>
            <div style={{ textAlign:"center", marginBottom:"3.5rem" }}>
              ...
              <h2 style={{ fontSize:"clamp(1.8rem,3vw,2.6rem)", fontWeight:700, color:DARK }}>How We <span style={{ color:BLUE }}>Work</span></h2>
            </div>
          </Anim>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:"1.5rem" }}>
            {VALUES.map((v,i)=>(
              <Anim key={v.title} delay={i*.1}>
                <div style={{ background:i===1?BLUE:WHITE, border:`1.5px solid ${i===1?BLUE:"#E5EAF5"}`, borderRadius:18, padding:"2rem", transition:"transform .25s,box-shadow .25s", cursor:"default" }}
      ...
      {/* ── PORTFOLIO PREVIEW ── */}
      <section style={{ padding:"7rem 3rem", background:BG }}>
        <div style={{ maxWidth:1920, margin:"0 auto" }}>
          <Anim>
            <div style={{ display:"flex", alignItems:"flex-end", justifyContent:"space-between", marginBottom:"3rem", flexWrap:"wrap", gap:"1rem" }}>
              <div>
                <Label>KARYA KAMI</Label>
                <h2 style={{ fontSize:"clamp(1.8rem,3vw,2.6rem)", fontWeight:700, color:DARK }}>Project <span style={{ color:BLUE }}>Kami.</span></h2>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(2,1fr)", gap:"1.25rem" }}>
            {(() => {
              ...
              if (fallback.length === 0) {
                // Fallback statis kalau database kosong
          ...
          <Anim>
            <div style={{ display:"flex", alignItems:"flex-end", justifyContent:"space-between", marginBottom:"3rem", flexWrap:"wrap", gap:"1rem" }}>
              <div>
                ...
                <h2 style={{ fontSize:"clamp(1.8rem,3vw,2.6rem)", fontWeight:700, color:DARK }}>Layanan <span style={{ color:BLUE }}>Kami</span></h2>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:"1.25rem" }}>
              {previewServices.map((svc, i) => (
                <Anim key={svc.id} delay={i*.1}>
                  <div onClick={onGoJasa} style={{ background:BG, border:"1.5px solid #E5EAF5", borderRadius:16, padding:"1.5rem", display:"flex", gap:"1.5rem", cursor:"pointer", alignItems:"center", transition:"border-color .25s,transform .25s,box-shadow .25s" }}
                    ...
                    <div style={{ width:140, height:96, borderRadius:12, flexShrink:0, background:svc.imgBg || "linear-gradient(135deg,#1B4FD8,#23d5ab)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:36 }}>{svc.emoji || svc.icon}</div>
                    <div style={{ flex:1 }}>
                    ...
                    <div style={{ flexShrink:0, color:BLUE, fontSize:20 }}>→</div>
                  </div>
                </Anim>
              ))}
            </div>
      ...
      <section style={{ padding:"4rem 3rem 6rem" }}>
        <Anim>
          <div style={{ maxWidth:1920, margin:"0 auto", background:BLUE, borderRadius:24, padding:"4.5rem 3rem", textAlign:"center", position:"relative", overflow:"hidden" }}>
            ...
            <div style={{ position:"relative" }}>
              ...
              <h2 style={{ fontSize:"clamp(1.8rem,3vw,2.6rem)", fontWeight:700, color:WHITE, marginBottom:"1rem" }}>Siap Memulai Proyek Bersama?</h2>
      ...
      {/* ── FOOTER ── */}
      <footer style={{ background:DARK, color:WHITE, padding:"4rem 3rem 2rem" }}>
        <div style={{ maxWidth:1920, margin:"0 auto" }}>
          ...
          <div style={{ borderTop:"1px solid rgba(255,255,255,.08)", paddingTop:"1.5rem", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
